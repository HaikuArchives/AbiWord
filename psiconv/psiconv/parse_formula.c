...
static int psiconv_parse_sheet_ref(const psiconv_buffer buf,int lev,
                                   ...
                                   psiconv_sheet_ref_t *result)
{
  ...
  psiconv_progress(lev+1,off,"Going to read a sheet ref");
  psiconv_progress(lev+2,off,"Going to read the offset encoding");
  temp = psiconv_read_u16(buf,lev+2,off,&res);
  if (res) {
    if (length)
      *length = 0;
    return res;
  }
  psiconv_debug(lev+2,off,"Encoded word: %04x",temp);
  result->absolute = (temp & 0x4000)?psiconv_bool_true:psiconv_bool_false;
  result->offset = (temp & 0x3fff) * ((temp & 0x8000)?-1:1);
  psiconv_debug(lev+2,off,"Reference: %s offset %d",
...
static int psiconv_parse_sheet_cell_reference(const psiconv_buffer buf,int lev,
                                         psiconv_u32 off, int *length,
                                         psiconv_sheet_cell_reference_t *result)
{
  ...
  psiconv_progress(lev+2,off+len,"Going to read the row reference");
  if ((res = psiconv_parse_sheet_ref(buf,lev+2,off+len,&leng,&result->row)))
    goto ERROR;
  len += leng;
  psiconv_progress(lev+2,off+len,"Going to read the column reference");
  if ((res = psiconv_parse_sheet_ref(buf,lev+2,off+len,&leng,&result->column)))
    goto ERROR;
  len += leng;
  ...
  if (res)
    goto ERROR;
  if (temp != 0) {
    psiconv_warn(lev+2,off+len,"Unknown byte in cell reference (ignored");
    psiconv_debug(lev+2,off+len,"Trailing byte: %02x",temp);
  }
  len ++;
...
static int psiconv_parse_sheet_cell_block(const psiconv_buffer buf,int lev,
                                         psiconv_u32 off, int *length,
                                         psiconv_sheet_cell_block_t *result)
{
  ...
  psiconv_progress(lev+1,off+len,"Going to read a sheet cell block");
  psiconv_progress(lev+2,off+len,"Going to read the first cell");
  if ((res = psiconv_parse_sheet_cell_reference(buf,lev+2,off+len,&leng,
                                                &result->first)))
    goto ERROR;
  len += leng;
  psiconv_progress(lev+2,off+len,"Going to read the last cell");
  if ((res = psiconv_parse_sheet_cell_reference(buf,lev+2,off+len,&leng,
                                                &result->last)))
    goto ERROR;
  len += leng;
...
static int psiconv_parse_formula_element_list(const psiconv_buffer buf, int lev,
                                       psiconv_u32 off, int *length, 
                                       ...
                                       psiconv_u32 maxlen)
{
  ...
  psiconv_progress(lev+1,off,"Going to read a formula element list");
  ...
  while (!eof && len+off < maxlen) {
    psiconv_progress(lev+3,off+len,"Going to read a formula item marker");
    marker = psiconv_read_u8(buf,lev+2,off+len,&res);
    if (res)
      goto ERROR8;
    psiconv_debug(lev+3,off+len,"Marker: %02x (%s)",marker,
                  formula_elements[marker].name);
    len ++;

    if (formula_elements[marker].formula_type == psiconv_formula_unknown) {
      psiconv_warn(lev+3,off+len,"Unknown formula marker found!");
      goto ERROR8;
      ...
      len--;
      psiconv_progress(lev+3,off+len,"End of this formula list");
      eof = 1;
    } else if (formula_elements[marker].formula_type == 
                                                 psiconv_formula_dat_int) {
      psiconv_progress(lev+3,off+len,"Next item: an integer");
      formula->data.dat_int = psiconv_read_u32(buf,lev+2,off+len,&res);
      if (res)
        goto ERROR8;
      formula->type = formula_elements[marker].formula_type;
      psiconv_debug(lev+3,off+len,"Value: %08x",formula->data.dat_int);
      len += 4;
      ...
      psiconv_progress(lev+3,off+len,"Next item: a float");
      formula->data.dat_float = psiconv_read_float(buf,lev+2,off+len,&leng,
                                                   &res);
      if (res)
        goto ERROR8;
      formula->type = formula_elements[marker].formula_type;
      psiconv_debug(lev+3,off+len,"Value: %f",formula->data.dat_float);
      len += leng;
      if ((res = psiconv_list_add(formula_stack,formula)))
        goto ERROR8;
      formula->type = psiconv_formula_unknown;
      ...
      psiconv_progress(lev+3,off+len,"Next item: a cell reference");
      if ((res = psiconv_parse_sheet_cell_reference(buf,lev+2,off+len,&leng,
      ...
      psiconv_progress(lev+3,off+len,"Next item: a cell block");
      if ((res = psiconv_parse_sheet_cell_block(buf,lev+2,off+len,&leng,
      ...
      psiconv_progress(lev+3,off+len,"Next item: a string");
      formula->data.dat_string = 
                    psiconv_read_short_string(buf,lev+2,off+len,&leng,&res);
      if (res)
        goto ERROR8;
      formula->type = formula_elements[marker].formula_type;
      len += leng;
      ...
      psiconv_progress(lev+3,off+len,"Next item: a variable reference");
      formula->data.dat_variable = psiconv_read_u32(buf,lev+2,off+len,&res);
      if (res)
        goto ERROR8;
      formula->type = formula_elements[marker].formula_type;
      len += 4;
      ...
      psiconv_progress(lev+3,off+len,"Going to parse a vararg function");
      ...
      do {
        nr_of_subs ++;
        psiconv_progress(lev+4,off+len,"Going to read vararg argument %d",
                         nr_of_subs);
        if ((res = psiconv_parse_formula_element_list(buf,lev+4,off+len,&leng,
                                                      &subformula,maxlen)))
          goto ERROR8;
        len += leng;
        ...
        free(subformula);
        psiconv_progress(lev+4,off+len,"Going to read the next marker");
        submarker = psiconv_read_u8(buf,lev+4,off+len,&res);
        len ++;
        if (res)
          goto ERROR8;
        submarker2 = psiconv_read_u8(buf,lev+4,off+len,&res);
        if (res)
          goto ERROR8;
      ...
      if (formula_elements[submarker].formula_type 
                                             != psiconv_formula_mark_opend) {
        psiconv_warn(lev+3,off+len,"Formula corrupted!");
        psiconv_debug(lev+3,off+len,"Found unexpected marker %02x",submarker);
        goto ERROR8;
      }
      psiconv_progress(lev+3,off+len,"Going to read the repeated marker %02x",
                       marker);
      submarker = psiconv_read_u8(buf,lev+3,off+len,&res);
      if (res)
        goto ERROR8;
      if (submarker != marker) {
        psiconv_warn(lev+3,off+len,"Formula corrupted!");
        psiconv_debug(lev+3,off+len,"Expected marker %02x, found %02x",
                      marker,submarker);
        goto ERROR8;
      }
      len++;
      psiconv_progress(lev+3,off+len,
                       ...
                       nr_of_subs);
      temp = psiconv_read_u16(buf,lev+3,off+len,&res);
      if (res)
        goto ERROR8;
      if (temp != nr_of_subs) {
        psiconv_warn(lev+3,off+len,"Formula corrupted!");
  ...
  if ((len+off > maxlen) || !eof) {
    psiconv_warn(lev+2,off+len,"Formula corrupted!");
    psiconv_debug(lev+2,off+len,"Expected end: %04x, found end: %04x",
                  maxlen,len+off);
    goto ERROR8;
  }
  if ((psiconv_list_length(formula_stack)) != 1) {
    psiconv_warn(lev+2,off+len,"Formula corrupted!");
    psiconv_debug(lev+2,off+len,"More than one item left on the stack (%d)",
...
ERROR2:
  free (*result);
ERROR1:
  psiconv_warn(lev+1,off,"Reading of formula element list failed");
  if (length)
    *length = 0;
   if (!res)
...
int psiconv_parse_formula(const psiconv_buffer buf, int lev,
                          psiconv_u32 off, int *length, 
                          psiconv_formula *result)
{
  int res=0;
  ...
  psiconv_progress(lev+1,off,"Going to read a formula");

  psiconv_progress(lev+2,off+len,
                   "Going to read the formula byte length");
  bytelen = psiconv_read_S(buf,lev+2,off+len,&leng,&res);
  if (res)
    goto ERROR1;
  psiconv_debug(lev+2,off+len,"Formula byte length: %d",bytelen);
  len += leng;
  bytelen += len;
  formula_end = off + bytelen;

  psiconv_progress(lev+2,off+len,"Going to read the formula elements list");
  if ((res = psiconv_parse_formula_element_list(buf,lev+2,off+len,&leng,
                                                result,formula_end)))
    goto ERROR1;
  len += leng;

  psiconv_progress(lev+2,off+len,"Going to read the eof marker");
  temp = psiconv_read_u8(buf,lev+2,off+len,&res);
  if (res)
    goto ERROR2;
  if (formula_elements[temp].formula_type != psiconv_formula_mark_eof) {
    psiconv_warn(lev+2,off+len,"Formula corrupted!");
    psiconv_debug(lev+2,off+len,"Expected marker: %02x, found byte: %02x",
                  0x15,temp);
    goto ERROR2;
  }
  len ++;
...
ERROR1:
  psiconv_warn(lev+1,off,"Reading of formula failed");
  if (length)
    *length = 0;
   if (!res)
     return -PSICONV_E_NOMEM;
