...
class ParagraphWin:public BWindow {
	public:
		...
		void SetDlg(AP_BeOSDialog_Paragraph *brk);
...
void ParagraphWin::SetDlg(AP_BeOSDialog_Paragraph *brk) 
{
	...
	{
		m_DlgParagraph->_createPreviewFromGC(m_BeOSGraphics,preview->Frame().Width(),preview->Frame().Height());
...
void AP_BeOSDialog_Paragraph::_checkBoxChanged(class BCheckBox* pBox)
{
		
	if(pBox == newwin->orphanControl)
		_setCheckItemValue(id_CHECK_WIDOW_ORPHAN ,(tCheckState)  (pBox->Value() == 1));
	else if(pBox == newwin->keepNext)
		_setCheckItemValue(id_CHECK_KEEP_NEXT , (tCheckState) (pBox->Value() == 1));
	else if(pBox == newwin->keepLines)
		_setCheckItemValue(id_CHECK_KEEP_LINES ,(tCheckState)  (pBox->Value() == 1));
	else if(pBox == newwin->pageBreak)
		_setCheckItemValue(id_CHECK_PAGE_BREAK ,(tCheckState)  (pBox->Value() == 1));
	else if(pBox == newwin->suppressLines)
		_setCheckItemValue(id_CHECK_SUPPRESS ,(tCheckState)  (pBox->Value() == 1));
	else if(pBox == newwin->noHyphenate)
		_setCheckItemValue(id_CHECK_NO_HYPHENATE ,(tCheckState)  (pBox->Value() == 1));
		
}


void AP_BeOSDialog_Paragraph::_menuChanged(uint32 ID , uint32 itemIndex)
{
#if 0
	int32 itemIndex;
	int32 numItems = srcMenu->CountItems();
	
	for(int itemIndex = 0; itemIndex < numItems; itemIndex ++)
	{
		if(srcMenu->FindMarked() == srcMenu->ItemAt(itemIndex))
			break;
	}
	...
	{
		case 'mens':
			_setMenuItemValue( AP_Dialog_Paragraph::id_MENU_SPECIAL_SPACING , itemIndex);
			break;

		case 'mena':
			_setMenuItemValue( AP_Dialog_Paragraph::id_MENU_ALIGNMENT , itemIndex);
			break;

		case 'mspe':
			_setMenuItemValue( AP_Dialog_Paragraph::id_MENU_SPECIAL_INDENT , itemIndex);
			break;
	}
}

#define ToggleCheckState(checkbox, truth) \
checkbox->SetValue(truth);

void AP_BeOSDialog_Paragraph::_populateWindowData()
{
	// set the check boxes
...
{ 
	...
	if (bAll || (changed == id_SPIN_SPECIAL_INDENT))
	{
		...
		if (_getMenuItemValue(id_MENU_SPECIAL_INDENT) == indent_FIRSTLINE)
		{
			...
			newwin->specialIndentation->Menu()->ItemAt(_getMenuItemValue(id_MENU_SPECIAL_INDENT))->SetMarked(true);//SendMessage(h, CB_SETCURSEL, (WPARAM) _getMenuItemValue(id_MENU_SPECIAL_INDENT), 0);
		}
	}
	if (bAll || (changed == id_MENU_SPECIAL_INDENT))
	{
		...
		case indent_NONE:
			...
			newwin->byIndentation->SetText("");//SetDlgItemText(m_hwndSpacing, AP_RID_DIALOG_PARA_EDIT_BY, NULL);
			break;

		default:
			// set the spin control
			newwin->byIndentation->SetText( _getSpinItemValue(id_SPIN_SPECIAL_INDENT));
	...
	if (bAll || (changed == id_SPIN_SPECIAL_SPACING))
	{
		...
		if (_getMenuItemValue(id_MENU_SPECIAL_SPACING) == spacing_MULTIPLE)
		{
			...
			newwin->lineSpacing->Menu()->ItemAt( _getMenuItemValue(id_MENU_SPECIAL_SPACING))->SetMarked(true);//SendMessage(h, CB_SETCURSEL, (WPARAM) _getMenuItemValue(id_MENU_SPECIAL_SPACING), 0);
		}
	}
	...
	if (bAll || (changed == id_MENU_SPECIAL_SPACING))
	{
		...
		default:
			...
			newwin->atSpacing->SetText(_getSpinItemValue(id_SPIN_SPECIAL_SPACING));
