...
bool EV_BeOSToolbar::toolbarEvent(XAP_Toolbar_Id id,
	...
	invokeToolbarMethod(m_pBeOSFrame->getFrame()->getCurrentView(),pEM,pData,dataLength);
	return true;
}
...
void EV_BeOSToolbar::hide() {
	UT_DEBUGMSG(("EV_BeOSToolbar::show()\n"));
	printf(("EV_BeOSToolbar::hide()\n"));
	if(!m_bHidden)
	{
		if(m_pTBView->Window()->Lock())
		{
			m_pTBView->Hide();
			m_pTBView->Window()->Unlock();
		}
