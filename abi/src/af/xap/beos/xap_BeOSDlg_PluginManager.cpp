...
#include "xap_BeOSDlg_PluginManager.h"

#include "ut_Rehydrate.h"

/*****************************************************************/

XAP_Dialog * XAP_BeOSDialog_PluginManager::static_constructor(XAP_DialogFactory * pFactory,
													 XAP_Dialog_Id id)
{
	XAP_BeOSDialog_PluginManager * p = new XAP_BeOSDialog_PluginManager(pFactory,id);
	return p;
}

XAP_BeOSDialog_PluginManager::XAP_BeOSDialog_PluginManager(XAP_DialogFactory * pDlgFactory,
												 XAP_Dialog_Id id)
	: XAP_Dialog_PluginManager(pDlgFactory,id)
{
}

XAP_BeOSDialog_PluginManager::~XAP_BeOSDialog_PluginManager(void)
{
}

void XAP_BeOSDialog_PluginManager::runModal(XAP_Frame * pFrame)
{
	UT_ASSERT(pFrame);
...
/*
	...
	2.  Do a case sensitive global replace on the words PluginManager and STUB
		in both files. 
