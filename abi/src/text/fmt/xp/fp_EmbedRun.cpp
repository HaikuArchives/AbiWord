...
// Load this into EmbedView
	...
	else
	{
	  ...
	  bool bFoundHeight = pSpanAP->getProperty("height",(const XML_Char *&)pszHeight);
	  const char * pszWidth = NULL;
	  bool bFoundWidth = pSpanAP->getProperty("width",(const XML_Char *&) pszWidth);
	  const char * pszAscent = NULL;
	  bool bFoundAscent = pSpanAP->getProperty("ascent",(const XML_Char *&) pszAscent);
	  ...
	  else
	  {
		  ...
		  else
		  {
			  ...
			  bool bFoundDescent = pSpanAP->getProperty("descent",(const XML_Char *&) pszDescent);
...
UT_sint32  fp_EmbedRun::_getLayoutPropFromObject(const char * szProp)
{
  ...
  if(pAP)
    {
      bool bFound = pAP->getProperty(szProp,(const XML_Char *&)szPropVal);
      if(bFound)
	{
...
bool fp_EmbedRun::_updatePropValuesIfNeeded(void)
{
  ...
  bool bFound = pAP->getProperty("height",(const XML_Char *&)szPropVal);
  ...
  bFound = pAP->getProperty("width",(const XML_Char *&)szPropVal);
  if(bFound && !bDoUpdate)
    {
      iVal = UT_convertToLogicalUnits(szPropVal);
  ...
  bFound = pAP->getProperty("ascent",(const XML_Char *&)szPropVal);
  if(bFound && !bDoUpdate)
    {
      iVal = UT_convertToLogicalUnits(szPropVal);
  ...
  bFound = pAP->getProperty("descent",(const XML_Char *&)szPropVal);
  if(bFound && !bDoUpdate)
    {
      iVal = UT_convertToLogicalUnits(szPropVal);
